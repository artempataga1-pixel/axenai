      ...
      <div className="mob-col" style={{ position: 'relative', zIndex: 10, maxWidth: '1100px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '4rem', flexWrap: 'wrap' }}>
        ...
        <div className="mob-full" style={{ flex: 1, minWidth: '300px', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(40px)', transition: 'opacity 0.8s, transform 0.8s' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', marginBottom: '1.5rem', padding: '6px 16px', border: '1px solid rgba(0,229,255,0.3)', borderRadius: '9999px', background: 'rgba(0,180,255,0.06)' }}>
            ...
            <span style={{ color: '#00e5ff', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', letterSpacing: '0.2em' }}>ДОКАЗАНО РЕЗУЛЬТАТАМИ</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '2rem', marginBottom: '2.5rem', flexWrap: 'wrap' }}>
            {stats.map((s, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '2rem', fontWeight: 900, color: '#00e5ff', lineHeight: 1, textShadow: '0 0 30px rgba(0,229,255,0.8), 0 0 60px rgba(0,229,255,0.4)' }}>{s.value}</span>
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.6rem', letterSpacing: '0.15em', color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase' }}>{s.label}</span>
              </div>
            ))}
          </div>
